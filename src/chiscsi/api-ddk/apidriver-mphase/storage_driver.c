...
char    itarget_vendor_id[VENDOR_ID_MAX + 1] =
        VENDOR_ID;
...
static void calculate_residual_cnt(chiscsi_scsi_command *sc, unsigned int len)
{
        if (len != sc->sc_xfer_len) {
                if (len >  sc->sc_xfer_len) {
	//		printk("%s: setting overflow buflen %u\n",__func__, len);
        		...
        		sc->sc_xfer_len = len;
	//		printk("%s: setting underflow buflen %u\n",__func__, len);
                }
        }
}
...
unsigned char *scmd_get_data_buffer(chiscsi_scsi_command *sc)
{
	...
	storage_sglist *sgl = (storage_sglist *)sc_sgl->sgl_vecs;

	if (sc_sgl->sgl_vecs_nr) {
		memset(sgl->sgvec->sg_addr, 0, sgl->sgvec->sg_length);
		return (sgl->sgvec->sg_addr);
	}  else 
		return ((unsigned char *)&sgl->sgvec);
}
					 ...
					 unsigned char *data, int dlen)
{
	...
	if (sc_sgl->sgl_vecs_nr) {
		...
		for (i = 0; i < sc_sgl->sgl_vecs_nr; i++, sgl++) {
			int copy = MINIMUM(dlen, sgl->sgvec->sg_length);
			memcpy(sgl->sgvec->sg_addr, data + off, copy);
			off += copy;
		}
		if (off != dlen) {
		...
		if (debug) 
			display_byte_string("SCSI COMMAND", sgl->sgvec->sg_addr, dlen);
        } else {
                memcpy(&sgl->sgvec, data, dlen);
	}
...
int read_command_execute(iface_scmd_info *scmd_info, unsigned int boff,
                                unsigned int blen, unsigned long long pos,
				int res_cnt) 
{
	/* pass the scsi command to the backend from here*/
	/* copy data into the buffers */
	if (res_cnt)
		calculate_residual_cnt(scmd_info->sc, boff+blen);
	if (0) {
		printk("itt 0x%x, Backend read %u data at off %u from pos %llu \n",
			scmd_info->sc->sc_itt, blen, boff, pos);
	}
	return 0;
}


int write_command_execute(iface_scmd_info *scmd_info, unsigned int boff,
				unsigned int blen, unsigned long long pos) 
{
	/* pass the scsi command to the backend from here*/
	/* write data from sglist to the disk */
	if (0) {
		printk("itt 0x%x, Backend wrote %u data at off %u to pos %llu\n",
			scmd_info->sc->sc_itt, blen, boff, pos);
	}
	return 0;
}
...
int lun_inquiry(chiscsi_scsi_command * sc, unsigned char *cdb)
{
	...
	if (cdb[1] & 0x2) {	/* CMDDT bit set */
		...
		if (cdb[2] == 0) {	/* supported vital product data pages */
			...
			set_scsi_id(buf + 4, SCSI_SN_MAX, SCSI_SN_STR, sc->sc_lun);
			len = SCSI_SN_MAX + 4;
			rv = 0;
