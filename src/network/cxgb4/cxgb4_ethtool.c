...
static void collect_sge_port_stats(struct adapter *adap, struct port_info *p,
...
static void collect_channel_stats(struct adapter *adap,
				  struct channel_stats *s, u8 i)
{
	...
	REBASE_STATS(cpl_stats, adap->tp_cpl_stats_base, u32);
	REBASE_STATS(err_stats, adap->tp_err_stats_base, u32);
	REBASE_STATS(fcoe_stats, pi->fcoe_stats_base, u32);
		      ...
		      u64 *data)
{
	...
	*data++ = (u64)pi->port_id;
	...
	p0 = STATS_START(lb_stats, u64);
	while (p0 < STATS_END(lb_stats, u64))
...
static int set_settings(struct net_device *dev, struct ethtool_cmd *cmd)
{
	 ...
	 */
	ret = t4_link_l1cfg(adapter, adapter->mbox, pi->tx_chan, lc);
	if (ret)
		*lc = old_lc;
			  ...
			  struct ethtool_pauseparam *epause)
{
	...
	if (netif_running(dev))
		return t4_link_l1cfg(adapter, adapter->mbox, pi->tx_chan, lc);
	return 0;
}
...
static int eeprom_rd_phys(struct adapter *adap, unsigned int phys_addr, u32 *v)
{
	...
	return vaddr < 0 ? vaddr : t4_os_pci_read_seeprom(adap, vaddr, v);
}
...
static int eeprom_wr_phys(struct adapter *adap, unsigned int phys_addr, u32 v)
{
	...
	return vaddr < 0 ? vaddr : t4_os_pci_write_seeprom(adap, vaddr, v);
}
...
static int set_flash(struct net_device *netdev, struct ethtool_flash *ef)
{
	...
	ret = request_firmware(&fw, ef->data, adapter->pdev_dev);
	if (ret < 0)
		return ret;
	...
	ret = t4_fw_upgrade(adapter, mbox, fw->data, fw->size, /*force=*/true);
	release_firmware(fw);
	if (!ret)
		dev_info(adapter->pdev_dev, "loaded firmware %s\n",
			 ef->data);
	return ret;
}
#endif
...
#endif  /* Linux version 2.6.32 and above */
