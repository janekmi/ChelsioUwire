...
static int tcp_v4_connect_offload(struct sock *sk, struct sockaddr *uaddr, int addr_len)
{
...
int prepare_tcp_for_offload(void)
{
	...
	FIND_SYMBOL("rt6_bind_peer", rt6_bind_peer_p);
#endif
...
/* Modelled on iscsi_sw_tcp_data_ready() */
...
static int toe_bond_xmit_hash_policy_l2(struct net_device *dev,
                                        struct neighbour *neigh, int count)
{
					 ...
					 int count, bool is_ipv6)
{
	...
	if (is_ipv6) {
		...
		return (v6hash ^ neigh->ha[5] ^ dev->dev_addr[5]) % count;
        } else {
                return ((ntohl(saddr ^ daddr) & 0xffff) ^
                        (neigh->ha[5] ^ dev->dev_addr[5])) % count;
        }
}
					 ...
					 bool is_ipv6, u16 l4_prot)
{
	...
	if (is_ipv6) {
                if (l4_prot == IPPROTO_TCP || l4_prot == IPPROTO_UDP)
                        layer4_xor = ntohs(sport ^ dport);
		...
		return layer4_xor % count;
                ...
                return (layer4_xor ^
                        ((ntohl(saddr ^ daddr)) & 0xffff)) % count;
        }
}
