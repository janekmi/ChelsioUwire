...
static inline void ring_fl_db(struct adapter *adapter, struct sge_fl *fl)
{
	...
	if (fl->pend_cred >= FL_PER_EQ_UNIT) {
		...
		if (unlikely(fl->bar2_addr == NULL))
			t4_write_reg(adapter, T4VF_SGE_BASE_ADDR + A_SGE_VF_KDOORBELL,
				     V_QID(fl->cntxt_id) | val);
		else {
			writel(val | V_QID(fl->bar2_qid),
		   ...
		   const struct cpl_rx_pkt *pkt)
{
	...
	if (unlikely(pkt->vlan_ex)) {
		__vlan_hwaccel_put_ctag(skb, be16_to_cpu(pkt->vlan));
		rxq->stats.vlan_ex++;
	}
...
static int napi_rx_handler(struct napi_struct *napi, int budget)
{
	...
	if (unlikely(rspq->bar2_addr == NULL)) {
		...
		writel(val | V_INGRESSQID(rspq->bar2_qid),
...
static unsigned int process_intrq(struct adapter *adapter)
{
	...
	if (unlikely(intrq->bar2_addr == NULL)) {
		...
		writel(val | V_INGRESSQID(intrq->bar2_qid),
			   ...
			   unsigned int iqid)
{
	...
	cmd.dcaen_to_eqsize =
		cpu_to_be32(V_FW_EQ_ETH_CMD_FBMIN(X_FETCHBURSTMIN_64B) |
...
int t4vf_sge_init(struct adapter *adapter)
{
	...
	ingpadboundary = 1 << (G_INGPADBOUNDARY(sge_params->sge_control) +
			       ingpad_shift);
	...
	if (is_t4(adapter->params.chip))
		 ...
		 */
		ingpackboundary = G_INGPACKBOUNDARY(sge_params->sge_control2);
