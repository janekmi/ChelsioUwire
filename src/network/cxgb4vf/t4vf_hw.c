...
int t4vf_port_init(struct adapter *adapter, int pidx)
{
	...
	port_cmd.action_to_len16 =
		cpu_to_be32(V_FW_PORT_CMD_ACTION(FW_PORT_ACTION_GET_PORT_INFO) |
			    FW_LEN16(port_cmd));
	...
	pi->mdio_addr = (v & F_FW_PORT_CMD_MDIOCAP) ?
			G_FW_PORT_CMD_MDIOADDR(v) : -1;
	pi->port_type = G_FW_PORT_CMD_PTYPE(v);
...
int t4vf_handle_fw_rpl(struct adapter *adapter, const __be64 *rpl)
{
	...
	case FW_PORT_CMD: {
		...
		link_ok = (stat & F_FW_PORT_CMD_LSTATUS) != 0;
		speed = 0;
		fc = 0;
		...
		for_each_port(adapter, pidx) {
			...
			mod = G_FW_PORT_CMD_MODTYPE(stat);
			if (mod != pi->mod_type) {
				pi->mod_type = mod;
			...
			if (link_ok != lc->link_ok || speed != lc->speed ||
				...
				lc->speed = speed;
				lc->fc = fc;
				lc->supported = be16_to_cpu(port_cmd->u.info.pcap);
