...
extern struct net_device* bond_3ad_xmit_xor_select(int slave_agg_no,
	...
	bond_for_each_slave(bond, slave, i) {
		...
		case BOND_LINK_UP:
			...
			if (bond->params.mode == BOND_MODE_XOR ||
			    bond->params.mode == BOND_MODE_ROUNDROBIN)
				toe_failover(slave->dev->master, slave->dev,
					     TOE_LINK_UP, NULL);
				    ...
				    struct net_device *bond_dev)
{
	...
	case NETDEV_REGISTER:
		...
		break;
        case NETDEV_DOWN:
		toe_failover(bond_dev, event_bond->first_slave ?
			     event_bond->first_slave->dev : NULL , TOE_BOND_DOWN, NULL);
		break;
	case NETDEV_UP:
		toe_failover(bond_dev, event_bond->first_slave ?
			     event_bond->first_slave->dev : NULL , TOE_BOND_UP, NULL);
		break;
	default:
		break;
	}
...
static struct net_device *bond_xmit_roundrobin_select(int slave_no,
					struct net_device *bond_dev)
{
	struct bonding *bond = netdev_priv(bond_dev);
	struct net_device *slave_dev = NULL;
...
struct net_device *bond_xmit_xor_select(int slave_no, struct net_device *bond_dev)
{
	struct bonding *bond = netdev_priv(bond_dev);
	struct net_device *slave_dev = NULL;
	struct slave *slave, *start_at;
...
static int bond_xmit_xor(struct sk_buff *skb, struct net_device *bond_dev)
{
	...
	slave_dev = bond_xmit_xor_select(slave_no, bond_dev);

	if (slave_dev)
		res = bond_dev_queue_xmit(bond, skb, slave_dev);
...
static int __init bonding_init(void)
{
	...
	register_toe_bond_rr_select_cb(bond_xmit_roundrobin_select);
        ...
        register_toe_bond_8023AD_select_cb(bond_3ad_xmit_xor_select);
