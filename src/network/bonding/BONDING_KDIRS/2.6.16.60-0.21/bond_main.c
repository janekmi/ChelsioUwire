...
static int bond_close(struct net_device *bond_dev)
{
	...
	switch (bond->params.mode) {
	case BOND_MODE_8023AD:
		cancel_delayed_work(&(BOND_AD_INFO(bond).ad_work));
		break;
	case BOND_MODE_TLB:
	case BOND_MODE_ALB:
		cancel_delayed_work(&(BOND_ALB_INFO(bond).alb_work));
		break;
	default:
		break;
	}

	flush_workqueue(bond_wq);
