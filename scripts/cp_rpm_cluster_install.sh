...
if [ "$isInstallSet" == "yes" ]; then
	...
	if [ "$installOfed" == "yes" ]; then
		ssh root@$machineName "cd /var/tmp/$pkgName && python install.py -i all -c $confTune -o "
	else
		#echo "ssh root@$machineName \"cd /var/tmp/$pkgName && python install.py -i all -c $confTune \"" >> $logfile 
		ssh root@$machineName "cd /var/tmp/$pkgName && python install.py -i all -c $confTune "
